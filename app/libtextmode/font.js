const {white, bright_white, palette_4bit, rgb_to_imgdata, rgb_to_hex} = require("./palette");
...
function generate_font_canvas(bitmask, height, length) {
    ...
    const rgba = rgb_to_imgdata(bright_white);
...
class Font {
    ...
    draw(ctx, block, x, y) {
        ...
        if (block.fg_rgb) {
            const canvas = create_coloured_glyph(this.canvas, block.code, block.fg_rgb, this.width, this.height);
